...
public class DicomDecoder {
	...
	void addInfo(int tag, String value) throws IOException {
		...
		if (info!=null &&  tag!=ITEM) {
			...
			System.out.println(insertid + "##" + info);
			...
			try{
				...
				if(insertid==0){
					...
					insertid =  pacsDAO.saveFileName(fileName,connection,directory,invstid,lastmodified);
				}
				if(tag==524384){
					PacsDAO pacsDAO = new JDBCPacsDAO();
					String temp[] = info.split(":");
					modality = temp[1];
					int upd = pacsDAO.updateData(modality,"modality",insertid,connection);
					int modid = pacsDAO.getModid(modality,connection);
					int upmodid = pacsDAO.updateModid(modid,insertid,connection);
				}
				if(tag==1048608){
					PacsDAO pacsDAO = new JDBCPacsDAO();
					String temp[] = info.split(":");
					pid = temp[1];
					invstid = pid;
					int upd = pacsDAO.updateData(pid,"pid",insertid,connection);
				}
				if(tag==1048592){
					PacsDAO pacsDAO = new JDBCPacsDAO();
					String temp[] = info.split(":");
					pname = temp[1];
					int upd = pacsDAO.updateData(pname,"pname",insertid,connection);
				}
				if(tag==524320){
					PacsDAO pacsDAO = new JDBCPacsDAO();
					String temp[] = info.split(":");
					studydate = temp[1];
					int upd = pacsDAO.updateData(studydate,"studydate",insertid,connection);
				}
