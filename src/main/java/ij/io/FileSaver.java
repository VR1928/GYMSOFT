...
public class FileSaver {
	...
	//name = sd.getFileName();
		name = SaveDialog.setExtension(name, extension);
		if (name==null)
			return null;
	...
	public boolean saveAsTiff(String path) {
		...
		try {
			...
			IJ.showStatus("Opening: " + dir + filename);
 			DicomDecoder dd = new DicomDecoder(dir, filename);
 			dd.insertid = 1;
 			FileInfo fi = null;
 			...
 			System.out.println("img to save"+dd.invstid);
 			
 			//save image in HIS Server
 			...
 			try{
 				...
 				String invstid = "202";
 				boolean checkinvstid = pacsDAO.checkInvst(connection,invstid);
 				if(checkinvstid){
 					Pacs pacs = pacsDAO.getInvstigationDetails(invstid,connection);
 					pacs.setFilename("Pacs Report");
 					pacs.setDate(DateTimeUtils.getUKCurrentDataTime("IST"));
 					
 					boolean checkpacsimgid = pacsDAO.checkPacsImgId(invstid,connection);
 					
 					int imgid = 0;
 					if(checkpacsimgid){
 						imgid = pacsDAO.getPacsId(invstid,connection);
 					}else{
 						imgid  = pacsDAO.saveInvstID(invstid,pacs,connection);
 					}
 					
 					
 					// save pacs blopb image
 					 File imgfile = new File(path);
 					  
 					  FileInputStream fin = new FileInputStream(imgfile);
 					 
 					   PreparedStatement pre =
 					   connection.prepareStatement("insert into his_pacs_data (imgid, imgname, clientid, invstid) values(?,?,?,?) ");
 					 
 					   pre.setInt(1, imgid);
 					   pre.setBinaryStream(2,(InputStream)fin,(int)imgfile.length());
 					   pre.setString(3, pacs.getClientid());
 					   pre.setString(4, invstid);
	...
	public boolean saveAsPng(String path) {
		...
		updateImp(fi, FileInfo.IMAGEIO);
			...
			try{
				connection = Connection_provider.getconnection();
				PacsDAO pacsDAO = new JDBCPacsDAO();
				int insertedimgid = pacsDAO.getSelectedImgID(connection);
				
				connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/akdcnagpur","pranams","6qxi5x&)~XBZ");
				
				int selectedid = pacsDAO.getDicomSelectedId(connection);
				String invstid = pacsDAO.getSelectedDicomInvstid(selectedid,connection);
				boolean checkinvstid = pacsDAO.checkInvst(connection,invstid);
				if(checkinvstid){
					Pacs pacs = pacsDAO.getInvstigationDetails(invstid,connection);
					pacs.setFilename("Pacs Report");
					pacs.setDate(DateTimeUtils.getUKCurrentDataTime("IST"));
					pacs.setDoctype("Investigation");
				...
				JButton mb = null;
				JOptionPane.showMessageDialog(mb, "Image Saved!!");
