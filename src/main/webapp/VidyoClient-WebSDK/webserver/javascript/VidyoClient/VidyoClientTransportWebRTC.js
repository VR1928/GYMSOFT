...
const STREAM_TYPE_SHARE_PREVIEW = "sharepreview";
...
function VidyoClientWebRTC(t) {
    ...
    const LOCAL_SHARE_PREVIEW_SOURCE_ID = "share-preview-source-id";
    ...
    function AttachVideo(sourceId) {
        if (streamMapping_.hasOwnProperty(sourceId) &&
            ...
            var elemId = streamMapping_[sourceId]["elemId"];
            let streamType = streamMapping_[sourceId].type;
            ...
            _buildRenderer(elemId, streamType);


            var streamId = streamMapping_[sourceId]["streamId"];
            var videoElement = layoutEngine_[elemId].getVideoElement(streamType,  streamId);
            let videoStream = streamType === STREAM_TYPE_SHARE_PREVIEW ? localShareStream_[streamId] : videoStreams_[streamId];
            ...
            if (videoElement && videoStream) {
                ...
                layoutEngine_[elemId].show(streamType,  streamId, streamMapping_[sourceId].name);
            }
        }
    };
        ...
        for (i = 0; i < data.streams.length; i++) {
            ...
            switch(type){
              case STREAM_TYPE_PREVIEW:
                ...
                break;
              case STREAM_TYPE_SHARE_PREVIEW:
                CreateSourceIdEntryInStreamMappingAndAttachVideo({
                  sourceId: LOCAL_SHARE_PREVIEW_SOURCE_ID,
                  streamId: 0,
                  elemId: viewId,
                  type: STREAM_TYPE_SHARE_PREVIEW,
                  name: "Share Preview",
                  attached: false
                });
                break;
  ...
  function GetCameraConstraints(data) {
    ...
    this.callback = function(data) {
        ...
        switch(data.method) {
            ...
            case "VidyoWebRTCInitRenderer":
                VidyoWebRTCInitRenderer(data.viewId);
            break;
    ...
    function VidyoWebRTCInitRenderer(viewId) {
      if (layoutEngine_.hasOwnProperty(viewId)) {
        layoutEngine_[viewId].uninitialize();
        delete layoutEngine_[viewId];
        LogInfo("LayoutEngine: " + JSON.stringify(Object.keys(layoutEngine_)));
      }
      layoutEngine_[viewId] = new LayoutEngine(viewId);
    }

    function _buildRenderer(viewId, streamType) {
      var displayCropped = layoutEngineAttrs_.hasOwnProperty(viewId) ? layoutEngineAttrs_[viewId] : false;
      layoutEngine_[viewId].initialize(rendererType, displayCropped, streamType);
      let show_audio_meters = (streamType === STREAM_TYPE_SHARE || streamType === STREAM_TYPE_SHARE_PREVIEW) ? false : showAudioMeters_;
      layoutEngine_[viewId].showAudioMeters(show_audio_meters);
    }
...
function LayoutEngine(viewId) {
    ...
    var SHARE_PREVIEW_FRAME_ID = "_vidyoSharePreviewFrame";

    var SELF_NAME_ID = "_vidyoSelfName";
    var REMOTE_NAME_ID = "_vidyoRemoteName";
    var SHARE_PREVIEW_NAME_ID = "_vidyoSharePreviewElm";
    ...
    **/
    ...
    function init(rendererType, displayCropped, streamType) {
        ...
        if (rendererType === RENDERER_TYPE_TILES) {
          view.innerHTML = _buildVideoContainer_Tiles(streamType, displayCropped);
        } else {
          view.innerHTML = _buildVideoContainer_Composite();
        }
    ...
    function _buildVideoContainer_Composite(){
      NUM_ELEMENTS = 9;
    ...
    function _buildVideoContainer_Tiles(streamType, displayCropped){
      NUM_ELEMENTS = 1;
      if (displayCropped) {
      ...
      switch(streamType){
        ...
        case STREAM_TYPE_SHARE:
          layoutTemplate += FRAME.replace("<frameid>", viewId + REMOTE_FRAME_ID + 0).replace("<nameid>", viewId + REMOTE_NAME_ID + 0);
          break;
        case STREAM_TYPE_SHARE_PREVIEW:
          layoutTemplate += FRAME.replace('"frame video"', '"frame video sharepreview"').replace("<frameid>", viewId + SHARE_PREVIEW_FRAME_ID).replace("<nameid>", viewId + SHARE_PREVIEW_NAME_ID);
          break;
      }
    ...
    this.initialize = function(rendererType, displayCropped, streamType) {
        if (!initialized) {
            initialized = init(rendererType, displayCropped, streamType);
        }
    };
    ...
    function showSharePreview(name) { // geth: this method is not for Composite Layout since it does not call calculateLayout
      currentContext.selfViewMode = "None";
      let frame = document.getElementById(viewId + SHARE_PREVIEW_FRAME_ID);
      let elem = document.getElementById(viewId + SHARE_PREVIEW_NAME_ID);
      if (elem) {
        elem.innerHTML = name;
      }
    ...
    function hideSharePreview() { // geth: this method is not for Composite Layout since it does not call calculateLayout
      currentContext.selfViewMode = "None";
      let frame = document.getElementById(viewId + SHARE_PREVIEW_FRAME_ID);
      let elem = document.getElementById(viewId + SHARE_PREVIEW_NAME_ID);
      if (elem) {
    ...
    this.getVideoElement = function(type, index) {
        ...
        switch (type) {
          ...
          case STREAM_TYPE_SHARE_PREVIEW:
            frame = document.getElementById(viewId + SHARE_PREVIEW_FRAME_ID);
            break;
...
function VidyoClientTransportWebRTC(plugInObj, statusChangeHandler, callbackHandler, plugInDivId, callback){
    ...
    const LOGGEDIN_INACTIVITY_TIMEOUT = -1; // this kind of timeout is disabled for now
    var loggedInInactivityTimer = null;

    const LOGGEDOUT_TIMEOUT = 10 * 60 * 1000;
    ...
    var connectionTimedOut = function() {
        if (connectionState === "CONNECTED" || connectionState === "CONNECTING") {
            ...
            statusChangeCallback({status: "TIMEDOUT"});
        }
    };
    ...
    var startLoggedOutTimeout = function() {
      ...
      loggedOutTimer = window.setTimeout(connectionTimedOut, LOGGEDOUT_TIMEOUT);
    };
    ...
    var startLoggedInInactivityTimeout = function() {
      if(LOGGEDIN_INACTIVITY_TIMEOUT > 0) {
        stopLoggedInInactivityTimeout();
        loggedInInactivityTimer = window.setTimeout(connectionTimedOut, LOGGEDIN_INACTIVITY_TIMEOUT);
        listenForActivity();
      }
    };
            ...
            switch(evts[i].destination) {
                ...
                default:
                    if(evts[i].type === 'callDisconnected'){
                        VCUtils.onloadCallback({state: 'TIMEDOUT', description: 'Inactivity timeout'});
                        break;
                    }
            }
        }
