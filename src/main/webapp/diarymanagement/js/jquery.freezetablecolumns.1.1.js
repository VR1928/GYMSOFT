...
(function( $ ) {
  ...
  */
  $.fn.freezeTableColumnsLayout = function(options) {
	...
	var fwidth = 0;
	if(document.getElementById('fullCalander').checked==true){
		fwidth = 4000;
	}else{
		fwidth = 1100;
	}
    // calculate how big the tables want to be, given an "unlimited" space (50,000 in this case)
    $('#' + main_id + '-region1').children('div').width(fwidth);
    $('#' + main_id + '-region2').children('div').width(fwidth);
    $('#' + main_id + '-region3').children('div').width(fwidth);
    $('#' + main_id + '-region4').children('div').width(fwidth);
