...
jQuery(function($) {
 ...
 $.fn.MessageBox = function(weekname,daynumber,action,id,tdid,starttime,endtime,apmtduration,location,onlinebooking,time) {
 	...
 	if(action=="edit"){
 		...
 		document.getElementById('apmtheading').style.display = 'none';
 		...
 		document.getElementById('deletediaryid').style.display = '';
 		
 		document.getElementById('weekNameListdiv').style.display = 'none';
 		
 		/*var str = '<input class="btn btn-primary" type="button" value="Submit" onclick="$(this).saveSlot(\''+id+'\');"/>'
 	...
 	if(action=="save"){
 		document.getElementById('apmtheading').style.display = '';
 		document.getElementById('weekNameListdiv').style.display = '';
 		document.getElementById('deletediaryid').style.display = 'none';
 		
 		editappointmentid = 0;
 		$( "#appointmendiv3" ).modal("show");
 		//$( "#appointmendiv" ).dialog( "open" );
		//$('#appointmendiv').dialog('option', 'title', 'Add Appointment');
 		...
 		var str = '<input class = "buttons" type="button" value="Book" onclick="$(this).saveSlot(\''+0+'\');"/>'
 		str = str + '<input class = "buttons" type="button" value="Cancel" onclick="$(this).cancelSlot();"/>'
 		document.getElementById('submitslot').innerHTML = str;*/
...
function saveAppointmentSlot(id){
	...
	if(parseFloat(id)== 0){
		...
		var url = "saveslotDiaryMangent?commencing="+commencing+"&selectedDiaryUser="+selectedDiaryUser+"&location="+location+"&room="+room+"&sTime="+sTime+"&endTime="+endTime+"&apmtDuration="+apmtDuration+"&onlineBooking="+onlineBooking+"&selecteddiaryUserid="+selecteddiaryUserid+"&tdcode="+tdcode+"&weekname="+weekname+"&selectedid="+id+"&wholeweek="+wholeweek+"&weekNumber="+weekNumber+"&monday="+monday+"&tuesday="+tuesday+"&wednesday="+wednesday+"&thursday="+thursday+"&friday="+friday+"&saturday="+saturday+"&sunday="+sunday+" ";
	}else{
		var url = "saveslotDiaryMangent?commencing="+commencing+"&selectedDiaryUser="+selectedDiaryUser+"&location="+location+"&room="+room+"&sTime="+sTime+"&endTime="+endTime+"&apmtDuration="+apmtDuration+"&onlineBooking="+onlineBooking+"&selecteddiaryUserid="+selecteddiaryUserid+"&tdcode="+tdcode+"&weekname="+weekname+"&selectedid="+id+" ";
	}
...
function saveAppointmentSlotRequest(){
	if (req.readyState == 4) {
		if (req.status == 200) {
			//jAlert('success', 'Appointment slot has been set successfully!!', 'Success Dialog');
			tempAlert("Appointment slot has been set successfully.", 5000);
...
function deleteSlotRequest(){
	if (req.readyState == 4) {
		if (req.status == 200) {
			//jAlert('success', 'Appointment slot has been deleted successfully!!', 'Success Dialog');
			tempAlert("Appointment slot has been deleted successfully", 5000);
			window.location.reload();
			//goPreview(tempusername,tempdate,tempyear,tempdiaryuserid,temptdcode)
