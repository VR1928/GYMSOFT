...
.scroll{
   height: 56%;
	...
	var url= "savegeneraldietplanDietarydetails?diettimeshift="+diettimeshift+"";
 ...
 function saveGeneralDietRequest(){
	if (req.readyState == 4) {
		if (req.status == 200) {	  
			...
			document.getElementById("diettimeshift").submit();
		}  
			if(diettimeshift1='0'){
			jAlert('success', 'Sccessfully Added!', 'Success Dialog');
...
<div class="">
 <form class="form-inline" id="ad4" action="savegeneraldietplanDietarydetails" method="post">
 <s:hidden name="allward" id="allward" />
				...
				<button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" style="width:100%;">Select Bed/Ward <span class="caret"></span></button>
												<ul class="dropdown-menu">
													<s:iterator value="wardlistname">
												 	 <li><a href="#" class="small" data-value="option1" tabIndex="-1"><input type="checkbox" id="p<s:property value="id"/>" class="pacss" value="<s:property value="id" />"/>&nbsp;<span class="spandrop"><s:property value="wardname"/></span></a></li>
												  </s:iterator>
												 
												</ul>
			</div> 
			<div class="form-group">
												<button type="button" class="btn btn-primary" onclick="wardfilter()">Go</button>
											</div>
											
											 <div class="form-group">
                                    <label class="sr-only" for="exampleInputPassword3">Date</label>
                                    <s:textfield name="fromdate" cssClass="form-control" id="fromdate" placeholder="Select Date"></s:textfield>
                            	</div>
		...
		<a class="btn btn-primary" href="#" onclick="saveGeneralDiet()" style="float: right; margin-top:5px;"><i
				class="fa fa-plus"></i> Save Diet Plan</a>
                           	
              					 <select name="diettimeshift" id="diettimeshift" class="form-control">
			    ...
			    <label>Dietician Incharge</label><label class="text-danger">*</label>
				<s:textfield id="dietician_incharge" name="dietician_incharge"
					cssClass="showToolTip form-control" data-toggle="tooltip"
					title="Enter Incharge Name" placeholder="Enter Incharge Name"/>
                    ...
                    <div class="row">
                        ...
                        <table class="table my-table xlstable table-bordered" style="width: 100%;">
                            ...
                            <s:iterator value="bedlist">
              				<%String coString="";%>
              				<%-- <s:if test="todayadmit==1">
              				<%coString="#00ff00"; %>
              				<img src="dashboardicon/newdiet.gif"></img>
              				</s:if> --%>
              				<s:if test='status=="0"'>
              				<tr style="background-color: <%=coString %>" bgcolor="#d5e1df">
              				</s:if>
              				<s:else>
              				<tr style="background-color: <%=coString %>" >
              				</s:else>
              				<td><%=i+1%>
              					<s:if test="todayadmit==1">
              				<%-- <%coString="#00ff00"; %> --%>
   ...
   <s:iterator value="categoryList">
				         <s:if test="checked==true">
				             <input class="case" type="checkbox" value="<s:property value="id"/>" checked="checked"/> <s:property value="name"/> <br>
				         </s:if>
				         <s:else>
				            <input class="case" type="checkbox" value="<s:property value="id"/>"/> <s:property value="name"/> <br>
				         </s:else>
				     </s:iterator> --%>
              					  ...
              					  <td>
              					  <input type="hidden" class="myclass" id="diaetplan<%=i %>" value="<%=i %>" name="diaetplan<s:property value="id"/>" />
              					<div class="scrolldietplan">
              					  <s:iterator value="catlist">
                          <s:if test="status==1">
                           <input checked="checked" type="checkbox" value="<s:property value="id"/>" class="fcase<%=i%>" /> <s:property value="name"/> <br>
                           </s:if>
                           <s:else>
                            <input type="checkbox" value="<s:property value="id"/>" class="fcase<%=i%>" /> <s:property value="name"/> <br>
              				...
              				<%-- 	 <td
              					 >
              					  <select name="diaetplan<s:property value="id"/>" onchange="selectDietPlan(this.value,id)" class="form-control">
              					 
              					  <s:iterator value="categoryList"><option value="0">Select Diet</option>
     ...
     <%--          				
                       	 ...
                       	 <form class="form-inline" id="addform" action="generaldietplanDietarydetails" method="post">
                       	 <s:hidden name="allward" id="allward1" />
                       	 </form>
                        </div>
